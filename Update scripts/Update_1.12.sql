...
SET NOCOUNT ON;
	...
	DECLARE @localQuery VARCHAR(MAX) = '
		SELECT
			...
			(SELECT Text FROM [N].[FundDescriptionLevel] fdl where fdl.Code = (SELECT DescriptionLevelCode FROM Funds f where f.SystemIdentifier = d.FundSystemIdentifier)) as LevelOfDescription,
			'''' as DocumentLink,
			a.Name as ArchiveName,
			(SELECT Code FROM [Archives] a where a.Id = d.ArchiveId) as ArchiveCode,
			d.Id as SystemId,
			(SELECT Number FROM Funds f where f.SystemIdentifier = d.FundSystemIdentifier) as FundNumber,
			(SELECT Number FROM Inventories i where i.SystemIdentifier = d.InventorySystemIdentifier) as InventoryNumber,
			(SELECT Number FROM ArchivalEntities ae where ae.SystemIdentifier = d.ArchivalEntitySystemIdentifier) as ArchiveEntityNumber,
			(CAST(d.StartSheetNumber AS nvarchar(50)) + '' - '' + CAST(d.EndSheetNumber AS nvarchar(50))) as ListNumbers, -- todo: различава се от ИСДА
			...
			(SELECT NumberNumeric FROM Funds f where f.SystemIdentifier = d.FundSystemIdentifier) as FundIntNumber,
			(SELECT NumberNumeric FROM Inventories i where i.SystemIdentifier = d.InventorySystemIdentifier) as InventoryIntNumber,
			(SELECT NumberNumeric FROM ArchivalEntities ae where ae.SystemIdentifier = d.ArchivalEntitySystemIdentifier) as ArchivalEntityIntNumber,
			a.SortOrder
		FROM Documents d
		INNER JOIN Archives a ON a.Id = d.ArchiveId AND a.Deleted = 0
		WHERE d.ExternalIdentifier IS NULL AND d.HasExternalSource = 0 AND d.Deleted = 0 
...
if not exists (select null from sys.columns where name = 'EventId' and object_id = object_id ('Notification.Notification'))
begin 
	alter table Notification.Notification add EventId int 
end
go


if object_id ('Notification.FK_Notification_Event') is null
begin
	alter table Notification.Notification
	add constraint FK_Notification_Event FOREIGN KEY ( EventId ) references Notification.NotificationEvent (Id)
end
go

update Notification.NotificationTemplate
set Body = N'<p>Регистриран е опис по Вашето #applicationType# с номер #applicationNumber#. Следва да добавите пакети А и Б към заявлението!</p>'
where NotificationTypeCode = 'AddApplicationPackages'
go
