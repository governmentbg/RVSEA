...
namespace DAA.Shared.Excel
{
    public static class ExportModelHelper
    {
        ...
        public static PropertyInfo[] GetMembersToValidateAgainstList<T>()
        {
            PropertyInfo[] membersToInclude = typeof(T)
                .GetProperties()
                .Where(p => p.GetCustomAttributes(typeof(ValidateAgainstListAttribute), true).Any())
                .ToArray();

            return membersToInclude;
        }

        public static ExcelDropDownListType GetPropImportListType<T>(string propName)
        {
            PropertyInfo propInfo = typeof(T).GetProperty(propName);
            if (propInfo != null)
            {
                object[] attrInfo = propInfo.GetCustomAttributes(typeof(ValidateAgainstListAttribute), false);
                if (attrInfo != null && attrInfo.Length > 0)
                    return (attrInfo[0] as ValidateAgainstListAttribute).FKSourceName;
            }
