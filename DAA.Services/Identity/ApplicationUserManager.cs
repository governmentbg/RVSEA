...
using DAA.Services.Identity;
using DAA.Shared.Localization;
using Microsoft.AspNetCore.Identity;
using Microsoft.Extensions.Localization;
using Microsoft.Extensions.Logging;
using Microsoft.Extensions.Options;
using DAAIdentityErrorDescriber = DAA.Services.Identity.IdentityErrorDescriber;
...
namespace DAA.Services.Identity
{
    public class ApplicationUserManager : UserManager<ApplicationUser>
    {
        ...
        /// of the operation.
        /// </returns>
        public virtual async Task<IdentityResult> AddToRoleByIdAsync(ApplicationUser user, Guid roleId)
        {
            ThrowIfDisposed();
            if (user == null)
            {
            ...
            if (await Store.IsInRoleByIdAsync(user, roleId, CancellationToken))
            {
                return UserAlreadyInRoleError(roleId.ToString("D"));
            }
            await Store.AddToRoleByIdAsync(user, roleId, CancellationToken);
            return await UpdateUserAsync(user);
        }
