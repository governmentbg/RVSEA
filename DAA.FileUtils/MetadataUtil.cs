...
namespace DAA.FileUtils
{
    public static class MetadataUtil
    {
        public static string GetFileMetadata(string fileName)
        {
            ...
            {
                // вероятно файлът е неподдържан формат - библиотеката работи за картинки, аудио, видео
            }
...
Ширина: {image.Width}
Височина: {image.Height}
DpiX: {Math.Round(image.HorizontalResolution)}
DpiY: {Math.Round(image.VerticalResolution)}
Цветове: {Image.GetPixelFormatSize(image.PixelFormat)}bit
            ");

            foreach (System.Drawing.Imaging.PropertyItem item in image.PropertyItems)
            {
                string propertyName = ConstsImaging.PropertyTagIds[item.Id];
                object? propertyValue = GetPropertyValue(item.Type, item.Value);
        ...
        // https://learn.microsoft.com/en-us/dotnet/desktop/winforms/advanced/how-to-read-image-metadata?view=netframeworkdesktop-4.8&redirectedfrom=MSDN
        // 1	A Byte
        ...
        private static object? GetPropertyValue(int propertyType, byte[]? bytes)
        {
            if (bytes == null)
            {
                return null;
    ...
    public static class ConstsImaging
    {
        public static Dictionary<int, string> PropertyTagIds = new()
        {
            { 0x0000, "GpsVer" },
