        ...
        private void LazyLoadDtd(Uri baseUri)
        {
            if (this.m_dtd == null && !this.m_ignoreDtd)
            {
                if (string.IsNullOrEmpty(this.m_syslit))
                {
                    ...
                    {
                        // този файл изглежда служи за валидиране, но и без него има някаква валидация на html
                        // спрях зареждането му защото имах хтмл страница, която беше невалидна според валидатора с пуснаата тази дефиниция (dtd
