using DAA.FileUtils.Watermark;
using Sgml;
using System.Drawing;
using System.Drawing.Imaging;
using System.Reflection;

/// source: https://satvasolutions.com/how-to-add-watermark-text-to-images-in-asp-net-using-c/
/// с някои поправки
///sad
///asd
///
...
namespace DAA.FileUtils.Watermark
{
...
namespace DAA.FileUtils
{
    ...
    {
        public static void AddWatermark(Stream srcStream, Stream destStream, ImageFormat imageFormat)
        {
            Image srcImage = Image.FromStream(srcStream);
            Image image = AddWatermarkHelp(srcImage, GetWatermarkImage());
            image.Save(destStream, imageFormat);
        }
        public static void AddWatermark(string srcFileName, Stream destStream, ImageFormat imageFormat)
        {
            Image srcImage = Image.FromFile(srcFileName);
            Image image = AddWatermarkHelp(srcImage, GetWatermarkImage());
            image.Save(destStream, imageFormat);
        }
        public static void AddWatermark(string srcFileName, string watermarkFileName, string destFileName)
        {
            Image srcImage = Image.FromFile(srcFileName);
            Image image = AddWatermarkHelp(srcImage, GetWatermarkImage());
        ...
        private static Stream GetWatermarkImage()
        {
            // така се виждат всички вградени ресурси
            //System.Reflection.Assembly.GetExecutingAssembly().GetManifestResourceNames(); 
            
            Assembly assembly = typeof(SgmlReader).Assembly;
            _ = assembly.FullName ?? throw new Exception("Assembly FullName not found!");
            string name = assembly.FullName.Split(',')[0] + ".Images.DAA-logo.png";
            return assembly.GetManifestResourceStream(name) ?? throw new Exception($"Embedded resource {name} not found!");
        }

        // 1 воден знак централно върху оригиналното изборажение с 80% от неговата големина
        private static Image AddWatermarkHelp(Image srcImage, Stream watermarkStream)
        {
            using Image watermarkImage = Image.FromStream(watermarkStream);
            Watermarker objWatermarker = new Watermarker(srcImage);

            objWatermarker.Position = WatermarkPosition.Center;
            float scaleX = (float)(objWatermarker.Image.Width * 0.8 / watermarkImage.Width);
            float scaleY = (float)(objWatermarker.Image.Height * 0.8 / watermarkImage.Height);
            objWatermarker.ScaleRatio = Math.Min(scaleX, scaleY);
            objWatermarker.DrawImage(watermarkImage);
            return objWatermarker.Image;
        }


        // подходящо воден знак с малък размер, който да се размножи по цялото основно изображение
        private static Image AddWatermarkHelpMultiply(Image srcImage, Stream watermarkStream)
        {
            using Image watermarkImage = Image.FromStream(watermarkStream);
            Watermarker objWatermarker = new Watermarker(srcImage);

            for (int i = 0; i < srcImage.Height; i += watermarkImage.Height * 2)
            {
                for (int j = 0; j < srcImage.Width; j += (int)watermarkImage.Width) // Width * 1.5 стара стойност
                {
                    objWatermarker.Position = WatermarkPosition.Center;
                    objWatermarker.PositionX = j;
                    objWatermarker.PositionY = i;
                    objWatermarker.Margin = new Padding(20);
                    objWatermarker.Opacity = 0.2f;
                    objWatermarker.TransparentColor = Color.Transparent;
                    objWatermarker.ScaleRatio = 0.9f;
...
//        for (int i = 0; i & lt; image.Height; i++)
//{
//    for (int j = 0; j & lt; image.Width; j++)
//{
