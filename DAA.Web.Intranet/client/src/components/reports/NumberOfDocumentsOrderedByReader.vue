<template>
    ...
    <v-container fluid class="overflowX">
        <v-card class="col-12 col-md-9 col-lg-9 ma-auto">
            <div class="card-cust">
                <h3 class="display-6">{{ t('reports.numberOfDocumentsOrderedByReaderReport') }}</h3>
                <Form @submit="viewReportOnSubmit">
                    <v-row align="center">
                        <v-col col="12" sm="12" xl="12" class="flexStart col-divs">
                            ...
                            <div :class="fieldSpacing">
                                <label>{{ t('reports.libraryCard') }}</label>
                                <Dropdown
                                    v-if="libraryCards"
                                    :label="t('reports.libraryCard')"
                                    name="fldLibaryCard"
                                    :items="libraryCards"
                                    v-model="inputModel.LibraryCardNumber"
                                    :selectAllText="t('common.selectAll')"
                                    ref="libraryCardsDropdown"
                                />
                            </div>
...
export default defineComponent({
    name: 'NumberOfDocumentsOrderedByReaderReport',
    emits: ['loadingChange'],
    props: {
        isCanceled: {
            type: Boolean,
    ...
    setup(_, { emit }) {
        ...
        const inputModel = ref(new NumberOfDocumentsOrderedByReaderReportFiltersModel());
        ...
        const libraryCardsDropdown = ref();
        const libraryCards = ref<IDropdownOption[]>();
        const getLibraryCards = async () => {
            libraryCards.value = await dropdownService.getLibraryCards();
        };

        const clear = () => {
            archivesDropdown.value.clear();
            fundLevelOfDescriptionsDropdown.value.clear();
            libraryCardsDropdown.value.clear();
        ...
        const onChange = (option: IDropdownOption) => {
            if (option) {
                ...
                libraryCardsDropdown.value?.clear();
                libraryCardsDropdown.value?.setDataSourceType(option.code);
            }
        };
        ...
        const viewReport = async (options: GridOptions) => {
            ...
            const gridInputModel = new ReportGridRequestModel<NumberOfDocumentsOrderedByReaderReportFiltersModel>({
                Page: options.page,
                ItemsPerPage: options.itemsPerPage,
                Filters: inputModel.value,
            });
            exportOptions.value = gridInputModel.Filters;
            exportParams.value = ReportType.numberOfDocumentsOrderedByReaderReport;

            await reportService
                .getNumberOfDocumentsOrderedByReaderReport(gridInputModel)
                .then(
                    (
        ...
        const gridItems = ref([] as NumberOfDocumentsOrderedByReaderReport[]);
        const headers = ref([
            {
            ...
            {
                title: t('users.columns.libraryCardNumber'),
        ...
        const summaryGridItems = ref([] as NumberOfDocumentsOrderedByReaderReportCombined[]);
        ...
        const breadcrumbItems = [
            ...
            {
                title: t('reports.numberOfDocumentsOrderedByReaderReport'),
                disabled: true,
            },
        ];
        return {
