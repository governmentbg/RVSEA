<template>
    ...
    <v-container fluid class="overflowX">
        <v-card class="col-12 col-md-9 col-lg-9 ma-auto">
            <div>
                <h3 class="display-6">{{ t('reports.inventoryBookOfCopiesFromForeignArchives') }}</h3>
...
export default defineComponent({
    name: 'InventoryBookOfCopiesFromForeignArchives',
    emits: ['loadingChange'],
    props: {
        isCanceled: {
            type: Boolean,
    ...
    setup(_, { emit }) {
        ...
        const viewReport = (options: GridOptions) => {
            ...
            exportParams.value = ReportType.inventoryBookOfCopiesFromForeignArchives;

            reportService
                .getInventoryBookOfCopiesFromForeignArchives(gridInputModel)
                .then(
                    (
                        result: ReportServiceResultModel<
                            ReportGridResponseModel<InventoryBookOfCopiesFromForeignArchives>
                        >
                    ) => {
        ...
        const gridItems = ref([] as InventoryBookOfCopiesFromForeignArchives[]);
        ...
        const headers = ref([
            ...
            {
                title: t('reports.countryOfOrigin'),
                prop: 'countryOfOrigin',
                type: 'string',
            },
            {
                title: t('films.columns.framesCount'),
                prop: 'framesCount',
                type: 'number',
            },
            {
                title: t('reports.microfilmNegativeRollsCount'),
                prop: 'microfilmNegativeRollsCount',
                type: 'number',
            },
            {
                title: t('reports.microfilmNegativeFramesCount'),
                prop: 'microfilmNegativeFramesCount',
                type: 'number',
            },
            {
                title: t('reports.microfilmPositiveRollsCount'),
                prop: 'microfilmPositiveRollsCount',
                type: 'number',
            },
            {
                title: t('reports.microfilmPositiveFramesCount'),
                prop: 'microfilmPositiveFramesCount',
                type: 'number',
            },
            {
            ...
            {
                title: t('reports.inventoryAvailability'),
                prop: 'hasInventory',
                type: 'boolean',
                renderFunction: formatYesNo,
            },
            {
                title: t('reports.shortDescription'),
                prop: 'inventoryShortDescription',
                type: 'string',
            },
            {
        ...
        const breadcrumbItems = [
            ...
            {
                title: t('reports.inventoryBookOfCopiesFromForeignArchives'),
                disabled: true,
            },
        ];
