<template>
    ...
    <v-card-text> -->
        <v-card class="col-12 col-md-9 col-lg-9 ma-auto">
            <div>
                <h3 class="display-6">{{ t('reports.fundsList') }}</h3>
...
export default defineComponent({
    ...
    setup(props, { emit }) {
        ...
        const gridItems = ref([] as FundsListReport[]);
        const summaryItems = ref([] as FundsListReportSummary[]);
        ...
        const viewReport = (options: GridOptions) => {
            ...
            exportParams.value = ReportType.fundsListInternal;

            reportService
                .getFundsListReport(gridInputModel)
                .then((result: ReportServiceResultModel<ReportGridResponseModel<FundsListReport>>) => {
                    gridItems.value = [];
                    gridItems.value = [...result.data!.items]; // това не е реактивно
                    ...
                    summaryItems.value = [];
                    const summary: FundsListReportSummary = {
                        totalRows: result.data!.totalCount
                    };
                    summaryItems.value.push(summary);
        ...
        const headers = ref([
            ...
            {
                title: t('reports.fundNameAndEndDatesOfFundCreator1'),
                prop: 'title',
                type: 'string',
            },
            {
