<template>
    <v-row align="center" v-if="searchEnabled">
        ...
        <v-col class="col-12 col-lg-4 d-flex justify-content-start">
            <v-btn @click="searchArchivalEntities" variant="flat">{{ t('common.search') }}
                <v-tooltip
                    activator="parent"
                    location="bottom"
                >
    ...
    <v-row>
        <v-col>
            <Pager
                v-if="archivalEntityData.totalCount > pagerOptions.itemsPerPage"
...
export default defineComponent({
    name: 'RelocationEnrollmentArchivalEntities',
    components: {
        TextField,
        Pager,
    },
    ...
    setup(props, context) {
        ...
        const getArchivalEntityData = async (keywords?: string, number?: string) => {
            try {
                if (keywords || number) {
                    initPagerOptions();
                }
                ...
                archivalEntityData.value = await archiveEntityService.getArchivalEntitiesByAvailabilityStatus(
                    pagerOptions.value,
        ...
        const searchArchivalEntities = async () => {
            await getArchivalEntityData(searchKeyword.value, searchNumber.value);
        };
