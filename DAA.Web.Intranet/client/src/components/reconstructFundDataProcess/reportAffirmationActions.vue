<template>
    <v-row align="end">
        <v-col class="d-grid gap-2 d-md-flex justify-content-center">
            <confirm-dialog 
                :confirmationText="t('processes.buttons.reconstructionModificationsAffirmationStepConfirmation')"
                :activatorButtonText="t('processes.buttons.modificationsAffirmationStep')"
                :confirmButtonText="t('common.yes')"
                :cancelButtonText="t('common.cancel')"
                @confirm="btnReportAffirmationStepClickHandler(ProcessStep.ReconstructFundData_Affirmation)"
            />
            ...
            <confirm-dialog 
                ...
                @confirm="(commentInput) => btnReportAffirmationStepClickHandler(ProcessStep.ReconstructFundData_ReportChangesRequired, commentInput)"
            />
        </v-col>
    </v-row>
</template>
...
export default defineComponent({
    name: 'ReconstructFundDataReportAffirmationActions',
    components: {
        ConfirmDialog,
    },
    props: {
    ...
    setup(props, context) {
        ...
        const btnReportAffirmationStepClickHandler = async (stepTypeId: number, comment?: string) => {
            try {
                await reconstructFundDataProcessService.sendModificationsAffirmationResult(
