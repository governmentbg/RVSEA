<template>
    ...
    <v-card class="col-12 col-md-9 col-lg-9 ma-auto">
        <v-card-title class="v-card-title-uppercase">{{ t('archiveEntities.edit') }}</v-card-title>
                ...
                <v-expansion-panels v-model="panel" multiple>
                        ...
                        <v-expansion-panel-text>
                            <v-row>
                                <v-col class="col-12">
                                    <TextField
                                        name="fldArchive"
                                        :label="t('archiveEntities.columns.archive')"
                            ...
                            <v-row>
                                <v-col class="col-12">
                                    <TextField
                                        name="fldInventory"
                                        :label="t('archiveEntities.columns.inventory')"
                            ...
                            <v-row>
                                <v-col class="col-12">
                                    ...
                                    <Dropdown
                                        ...
                                        :modelValue="archivalEntityData.descriptionLevelCode"
                                        required="required"
                            ...
                            <v-row>
                                <v-col class="col-12">
                                    <text-field
                                        ...
                                        :readonly="true"
                                    />
                                    <label v-if="chronologicalScopeLabelText" class="text-danger">{{
                    ...
                    <v-expansion-panel value="storage">
                        ...
                        <v-expansion-panel-text>
                            ...
                            <!-- <v-row>
                                <v-col class="col-12">
                                    <label for="fldOriginality">{{ t('archiveEntities.columns.originality') }}</label>
                                    <Dropdown
                            ...
                            <v-row>
                                <v-col class="col-12">
                                    <label for="fldCreationMethod">{{
                                        t('archiveEntities.columns.creationMethod')
                                    }}</label>
                                    <Dropdown
...
<script lang="ts">
import { defineComponent, ref, onMounted, Ref, inject, computed, watch } from 'vue';
...
import { ArchivalEntity, ArchivalEntityDraft } from '@/models/archivalEntity';
import { SearchedInventoryRequestModel, InventoryShort } from '@/models/inventory';
...
export default defineComponent({
    ...
    setup(props) {
        ...
        const goBack = () => {
            console.log('AE go back');
            //router.back();
            useRedirectWithId(router, 'DisplayArchiveEntity', props.id);
        };
        ...
        const preEditInventory = ref(new InventoryShort({}));
        ...
        const getarchivalEntityData = async () => {
            try {
                archivalEntityData.value = await archiveEntityService.displayArchivalEntity(props.id);
            } catch (error: unknown) {
        ...
        const changeInventories = (option: InventoryShort) => {
            if (option) {
                archivalEntityData.value.inventoryExternalIdentifier = option.externalIdentifier;
            }
        };

        const getInventories = async (searchText: string) => {
            const requestModel = new SearchedInventoryRequestModel({
                fundExternalIdentifier: archivalEntityData.value.fundExternalIdentifier,
                hasFundExternalSource: archivalEntityData.value.fundHasExternalSource,
        ...
        const submitarchivalEntityData = async () => {
            if (archivalEntityData.value) {
                try {
                    const archivalEntityDraft = new ArchivalEntityDraft(archivalEntityData.value);
                    archivalEntityDraft.isCurrent = true;
                    archivalEntityDraft.readOnly = false;
                    archivalEntityDraft.number = `${
                    ...
                    archivalEntityDraft.approxmateChronologicalScope = date.value;
                    switch (activeProcessData.value?.processTypeId) {
                        case ProcessType.EditData:
                            archivalEntityDraft.statusCode = Status.Modified;
                            break;
                        case ProcessType.RefineData:
                            archivalEntityDraft.statusCode = Status.Refined;
                            break;
                    }

                    const result = await archiveEntityService.updateArchivalEntity(archivalEntityDraft);
        ...
        const showInventoriesAsyncDropdown = ref(false);
        const asyncDropdownInventory = ref();
        const switchToAsyncDropdown = () => {
            showInventoriesAsyncDropdown.value = true;
            //asyncDropdownInventory.value.change();
        };
        ...
        return {
            ...
            goBack,
            showInventoriesAsyncDropdown,
            switchToAsyncDropdown,
            asyncDropdownInventory,
            preEditInventory,
            languages,
            creationMethods,
            originalities,
