...
export const getTotalUploadFileSize = (importedStructure: ImportedStructure[]) => {
...
export const getTotalUploadFileSizeForFiles = (files: File[]) => {
    const totalFileSize = files?.map(x => x.size ?? 0).reduce((acc, curr) => acc + curr, 0);
    return totalFileSize;
};
