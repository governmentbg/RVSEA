...
export const getters: GetterTree<UserState, UserState> & UserGetters = {
    ...
    hasRole(state): (value: string) => boolean {
        return (value) => {
            if (state.user) {
                return (state.user.isAdmin) || state.user.roles.find((role: string) => role === value) !== undefined
            }
            return false
        }

    },
    unseenNotifications(state): UINotificationModel[] {
        return state.user?.unseenNotifications || [];
    },
    refreshNotificationsPage(state): boolean {
        return state.user?.refreshNotificationsPage || false;
    }
}