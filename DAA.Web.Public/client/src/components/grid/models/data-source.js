...
export default class DataSource {
    ...
    _filterDataBySearch(data) {
        if (data && this.searchString) {
            ...
            if (this.searchString) {
                data = data.filter(function(row) {
                    var props = Object.getOwnPropertyNames(row.items);
    ...
    _processRemoteData() {
        ...
        return axios.post(this.baseUrl, options)
            .then(response => {
                let rows = [];
    ...
    addFilter(filter) {
        if (filter.type === "boolean") {
            filter.value = filter.col.filterByBooleanFunction(filter.value, filter.col.positiveValueText, filter.col.negativeValueText);
        }
