<template>
    <Loader :showCancel="true" @cancel="onCancleClick" :isLoading="isLoading" />
            ...
            <v-container>
                ...
                <v-row class="mt-3">
                    <v-col class="d-flex gap-2 justify-content-center">
                        ...
                        <v-btn class="btn-cancel" @click="clearCriteria">
                            {{ t('common.clear') }}
                            <v-tooltip activator="parent" location="bottom">
    ...
    <v-container v-if="!canSeeSearch" class="mx-auto search-results">
...
<script lang="ts">
import { defineComponent, onMounted, ref, watch, inject, Ref, computed } from 'vue';
...
export default defineComponent({
    ...
    setup() {
        ...
        const canSeeSearch = computed(() => userStore.getters.profileType === ProfileType.ReaderInReadingRoom);
        ...
        const getDescriptionLevels = async () => {
            try {
                descriptionLevels.value = await dropdownService.getAllDescLevelsForPublicSearch(message);
        ...
        const getSearchResultsData = async (options: GridOptions, changePage = false) => {
            try {
                ...
                splitCodes();
                searchResultsData.value = await searchService.getResultFromSearchCriteria(
                    searchModel.value as SearchModel
                );
        ...
        const changePage = async (pageNumber: number, pageSize: number) => {
            if (pageNumber != pagerOptions.value.page || pageSize != pagerOptions.value.itemsPerPage) {
                isLoading.value = true;
            }
...
.v-card {
    background: var(--ISDA-main-color2-1);
    border-radius: 6px;
    border-width: 1px;
}
...
:deep(.active a) {
    color: var(--ISDA-main-color1);
    background-color: var(--ISDA-main-color4);
    border-color: var(--ISDA-main-color4);
}
